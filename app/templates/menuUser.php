<h3 class="text">Bienvenido Usuario</h3>
<div class="container-fluid menu text-center p-3 my-4">
	<div class="container">
		<div class="row">
			<div class="col-md-12 ">
				<a href="index.php?ctl=editarPerfil" class="p-5">Edita Perfil</a> |
				<a href="index.php?ctl=paginaPrincipal" class="p-5">Pagina Principal</a> |
                <a href="index.php?ctl=listarServicios">Listar Servicios</a> |
                <a href="index.php?ctl=nuevoServicio">Nuevo Servicio</a> |
                <a href="index.php?ctl=cerrarSesion">Cerrar Sesion</a> |
			</div>
		</div>
	</div>
</div>